    ...
    const handleVerifyPayment = async (orderId: string) => {
        try {
            ...
            if (status === 'PAID' || status === 'SUCCESS') {
                // ✅ Trigger Automatic Activation RPC
                try {
                  const { error: rpcError } = await supabase.rpc('activate_personal_album', { _order_id: orderId });
                  if (rpcError) console.error('[DEBUG] RPC Activation Error:', rpcError);
                } catch (e) {
                  console.error('[DEBUG] RPC Trigger Failed:', e);
                }
