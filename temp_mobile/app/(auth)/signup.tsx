...
const SignupWorkflow = () => {
    ...
    const API_BASE = process.env.EXPO_PUBLIC_API_URL || (Platform.OS === 'web' ? 'http://localhost:8080' : '');
    ...
    const handlePostPayment = async (orderId: string, requestId: string) => {
        ...
        try {
            ...
            if (statusData.order_status === 'PAID' || statusData.order_status === 'ACTIVE') {
                 ...
                 const dashboardUrl = `http://${request.business_slug}.localhost:8080/dashboard`; 
        ...
        try {
            ...
            const response = await fetch(`${API_BASE}/api/create-cashfree-order`, {
                ...
                body: JSON.stringify({
                    ...
                    return_url: `${window.location.origin}/signup?request_id=${request.id}&order_id={order_id}`
                })
            });
