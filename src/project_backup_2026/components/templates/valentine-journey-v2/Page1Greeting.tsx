        ...
        <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-pink-500 via-rose-500 to-orange-500 flex items-center justify-center p-4">
            ...
            >
                ...
                <div className="bg-white rounded-3xl shadow-2xl p-8 pb-16 transform hover:rotate-0 transition-all duration-500 hover:shadow-pink-500/50">
                    ...
                    // Removed double click to prevent conflict with button click
                    >
                        <motion.img
                        ...
                        {isEditing && (
                            <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                {isUploading ? (
                                    ...
                                    <>
                                        <p className="text-white text-[10px] font-black uppercase tracking-widest mb-2">Change Photo</p>
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                fileInputRef.current?.click();
                                            }}
                                            className="bg-white text-pink-600 px-6 py-2 rounded-full font-black text-[10px] shadow-lg hover:scale-105 transition-all uppercase tracking-tighter flex items-center gap-2"
                                        >
                    ...
                    <div className="text-center space-y-4 mb-10">
                        <div
                            className={`relative group/text transition-all ${isEditing ? 'cursor-pointer p-4 hover:bg-rose-50 rounded-2xl border-2 border-transparent hover:border-rose-200' : ''}`}
                            onClick={() => {
                                if (isEditing) {
                        ...
                        >
                            ...
                            {isEditing && (
                                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-white px-3 py-1 rounded-full shadow-sm border border-pink-100 flex items-center gap-1">
                                    <span className="text-[10px] font-black text-pink-500 uppercase tracking-[0.2em]">✏️ Edit Greeting</span>
                                </div>
                            )}
                        </div>

                        <div
                            className={`relative group/name transition-all ${isEditing ? 'cursor-pointer p-6 hover:bg-rose-50 rounded-2xl border-2 border-transparent hover:border-rose-200' : ''}`}
                            onClick={() => {
                                if (isEditing) {
                        ...
                        >
                            ...
                            {isEditing && (
                                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-white px-3 py-1 rounded-full shadow-sm border border-pink-100 flex items-center gap-1">
                                    <span className="text-[10px] font-black text-pink-500 uppercase tracking-[0.2em]">✏️ Edit Name</span>
                                </div>
                            )}
                        </div>

                        <div
                            className={`relative group/sub transition-all ${isEditing ? 'cursor-pointer p-4 hover:bg-rose-50 rounded-2xl border-2 border-transparent hover:border-rose-200' : ''}`}
                            onClick={() => {
                                if (isEditing) {
                        ...
                        >
                            ...
                            {isEditing && (
                                <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-white px-3 py-1 rounded-full shadow-sm border border-pink-100 flex items-center gap-1">
                                    <span className="text-[10px] font-black text-pink-500 uppercase tracking-[0.2em]">✏️ Edit Subtext</span>
                                </div>
                            )}
                        </div>
                    </div>
