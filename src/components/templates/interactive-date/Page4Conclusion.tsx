...
const Page4Conclusion = ({ data, onNext, isEditing = false, onUpdate }: Page4ConclusionProps) => {
    ...
    return (
        <div className="min-h-screen relative overflow-hidden bg-[#051010] flex flex-col items-center justify-center p-8 text-center text-white">
            {/* Celebration Aura */}
            <motion.div
                animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.2, 0.1] }}
                transition={{ duration: 4, repeat: Infinity }}
                className="absolute w-[800px] h-[800px] bg-teal-600 blur-[150px] rounded-full"
            />
            ...
            <div className="relative z-10 max-w-4xl">
                ...
                >
                    <div className="relative">
                        <Heart size={160} fill="#0d9488" className="text-teal-600 drop-shadow-[0_0_80px_rgba(20,184,166,0.7)]" />
                        <motion.div
                            animate={{ scale: [1, 1.6], opacity: [0.4, 0] }}
                            transition={{ duration: 2, repeat: Infinity }}
                            className="absolute inset-0 bg-teal-500 rounded-full"
                        />
                        <Sparkles className="absolute -top-10 -right-10 text-teal-300 w-20 h-20 animate-pulse" />
                    </div>
                </motion.div>
                ...
                <div
                    ...
                    onDoubleClick={() => {
                        if (isEditing) {
                            const val = prompt("Edit Conclusion Message:", defaultData.msg);
                ...
                >
                    <button
                        ...
                        className="px-16 py-6 bg-white text-teal-600 font-black text-xs md:text-sm uppercase tracking-[0.6em] rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-all"
                    >
                        Plan Another Date
                    </button>
                    <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.5em] italic">
                        Counting down the seconds until we meet
                    </p>
                </motion.div>
            </div>
        </div>
