...
import { Utensils, Pizza, Soup, Beef, ShieldCheck, Zap, Sparkles, Star, ChevronRight, Menu, Check } from 'lucide-react';
...
const Page2Cuisine = ({ data, onNext, isEditing = false, onUpdate }: Page2CuisineProps) => {
    ...
    const defaultData = {
        heading: data.heading || "The Gastronomic Foundation",
        options: data.options && data.options.length > 0 ? data.options : [
            "Italian Palazzo",
            "Zen Garden Sushi",
            "Midtown Elite Grills",
            "Coastal Mexican Soul"
        ]
    };
    ...
    return (
        <div className="min-h-screen relative overflow-hidden bg-[#020606] flex flex-col items-center justify-center p-8 text-center font-outfit isolate selection-none">
            ...
            {/* HYPER-REALISTIC MENU SELECTION ATMOSPHERE (V2 Enhancement) */}
            <div className="fixed inset-0 z-0 pointer-events-none">
                <motion.div
                    ...
                    className="absolute inset-0 bg-[radial-gradient(circle_at_bottom_right,rgba(20,184,166,0.25),transparent_70%)]"
                />

                {/* Subtle Hexagon Grid */}
                <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/hexellence.png')] opacity-5 mix-blend-overlay" />

                {/* Ambient Spotlight */}
                <div className="absolute top-[-20%] left-1/2 -translate-x-1/2 w-[800px] h-[600px] bg-teal-500/10 rounded-full blur-[120px] mix-blend-screen" />
            </div>
            ...
            <div className="relative z-10 w-full max-w-7xl flex flex-col items-center gap-16">
                ...
                <div className="flex flex-col items-center gap-6">
                    ...
                    >
                        <Menu size={16} className="text-teal-400" />
                        <span className="text-teal-100/50 font-black uppercase tracking-[0.6em] text-[10px]">Course Selection // Primary</span>
                            ...
                            <div className="flex items-center gap-8 relative z-10 w-full">
                                ...
                                <div className="space-y-2 flex-grow">
                                    <div className="flex items-center gap-3">
                                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-teal-600">Signature Dish 0{index + 1}</p>
                ...
                <div className="h-24 flex items-center justify-center">
                    <AnimatePresence mode="wait">
                        {selected ? (
                            ...
                            >
                                <span className="text-[10px] font-black tracking-[0.8em] uppercase text-teal-100">Awaiting Preference</span>
                                <div className="flex gap-2">
                                    <div className="w-1 h-1 bg-teal-500 rounded-full animate-bounce" style={{ animationDelay: '0s' }} />
                                    <div className="w-1 h-1 bg-teal-500 rounded-full animate-bounce" style={{ animationDelay: '0.1s' }} />
                                    <div className="w-1 h-1 bg-teal-500 rounded-full animate-bounce" style={{ animationDelay: '0.2s' }} />
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
            ...
            <div className="fixed bottom-12 left-12 flex flex-col items-start gap-4 opacity-15">
                <div className="font-romantic text-8xl text-teal-900 italic">Menu</div>
                <div className="h-[1px] w-72 bg-teal-900/40" />
                <span className="text-[10px] font-black tracking-widest text-teal-200 uppercase tracking-[1.5em]">SELECTION-TYPE // LUXURY</span>
            </div>
