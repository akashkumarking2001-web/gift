...
const HeroSection = () => {
  ...
  const [specialOfferText, setSpecialOfferText] = useState("Valentine's Day Special Offer");

  useEffect(() => {
    const fetchSettings = async () => {
      const settings = await SettingsService.getSettings();
      if (settings.hero_special_offer) {
        setSpecialOfferText(settings.hero_special_offer);
      }
    };
    fetchSettings();
  }, []);
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center w-full">
        ...
        <div className="relative z-10 flex flex-col gap-8 text-left">
          ...
          >
            ...
            <span className="text-pink-200 text-xs font-bold uppercase tracking-widest">{specialOfferText}</span>
          </motion.div>
          ...
          >
            ...
            <button
              onClick={() => window.location.href = '/magic-frame/'}
