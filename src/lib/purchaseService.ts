...
export const PurchaseService = {
    ...
    async createPurchase(purchase: Omit<UserPurchase, 'id' | 'user_id' | 'user_email' | 'purchased_at'>): Promise<UserPurchase> {
        ...
        const { data: purchaseData, error: purchaseError } = await supabase
            ...
            .insert({
                ...
                status,
                is_bundle: false,
                cf_order_id: purchase.cf_order_id,
                cf_session_id: purchase.cf_session_id,
                payment_gateway_status: status === 'approved' ? 'PAID' : 'ACTIVE',
        ...
        // 2. Create template access record
        const { error: accessError } = await supabase
            ...
            .insert({
                ...
                is_locked: isLocked,
                unlocked_at: !isLocked ? new Date().toISOString() : null
            });
    ...
    async createBundlePurchase(bundlePurchase: Omit<UserPurchase, 'id' | 'user_id' | 'user_email' | 'purchased_at'>): Promise<UserPurchase> {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) throw new Error('User not authenticated');

        const status = bundlePurchase.status || 'pending';
        const isLocked = status !== 'approved';
        ...
        const { data: purchaseData, error: purchaseError } = await supabase
            ...
            .insert({
                ...
                is_bundle: true,
                status: status,
                cf_order_id: bundleConfig.cf_order_id,
                cf_session_id: bundleConfig.cf_session_id,
                payment_gateway_status: status === 'approved' ? 'PAID' : 'ACTIVE',
                approved_at: status === 'approved' ? new Date().toISOString() : null,
                approved_by: null
            })
        ...
        // 3. Create template access records for each template in the bundle
