...
const Editor = () => {
    ...
    const [pendingMedia, setPendingMedia] = useState<Map<string, File>>(new Map());
            ...
            try {
                ...
                if (foundTemplate) {
                    ...
                    if (auth.data.session) {
                        const access = await PurchaseService.hasTemplateAccess(foundTemplate.id.toString()) || 
                                     await PurchaseService.hasTemplateAccess(foundTemplate.slug);
                        setHasAccess(access);
                    }
                }
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, pageId: string, field: string) => {
        ...
        // Verify if user has paid. If not, only show local preview
        if (!hasAccess) {
           const file = files[0];
           const blobUrl = URL.createObjectURL(file);
           updateField(pageId, field, blobUrl);
           setPendingMedia(prev => new Map(prev).set(`${pageId}-${field}`, file));
           toast({ 
             title: "Preview Mode", 
             description: "You haven't purchased this template yet. Media will only be stored permanently after payment.",
             variant: "default"
           });
           e.target.value = '';
           return;
    ...
    const handlePublish = async () => {
        ...
        if (!hasAccess) {
            toast({ 
                ...
                description: "You must purchase this template to publish your gift and save media permanently.", 
                variant: "destructive" 
            });
            navigate(`/checkout?templateId=${template.id}&title=${encodeURIComponent(template.title)}&price=${template.price}`);
            return;
        }
        try {
    ...
    const handleShare = async () => {
        if (!hasAccess) {
            toast({ 
                ...
                description: "Please complete the purchase to share this gift.", 
                variant: "destructive" 
            });
            navigate(`/checkout?templateId=${template.id}&title=${encodeURIComponent(template.title)}&price=${template.price}`);
            return;
        ...
        <div className="min-h-screen bg-[#050505] relative overflow-hidden flex flex-col font-outfit text-white">
            ...
            <header className="relative z-30 bg-black/40 backdrop-blur-2xl border-b border-white/5 px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-6">
                    ...
                    <Link to="/">
                      <Logo iconSize="h-9 w-9" showText={false} />
                    </Link>
                    ...
                    <div>
                        ...
                        <h1 className="font-black text-lg tracking-tight leading-none mt-1">{template.title}</h1>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 md:gap-6">
                    ...
                    <div className="flex items-center gap-1.5 md:gap-3">
                        {hasAccess === false && (
                            <motion.button
                                onClick={() => navigate(`/checkout?templateId=${template.id}&title=${encodeURIComponent(template.title)}&price=${template.price}`)}
                                initial={{ opacity: 0, scale: 0.8 }}
                                animate={{ opacity: 1, scale: 1 }}
                                className="hidden md:flex items-center gap-2 px-4 py-2 bg-primary/20 border border-primary/30 text-primary rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-primary/30 transition-all"
                            >
                            ...
                            <div className="grid gap-8">
                                {activePage.requiredFields.map((field: string) => (
                                    <motion.div
                                        ...
                                        className="glass-card p-6 border border-white/5 hover:border-white/10 transition-all group/field"
                                    >
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center border border-primary/20">
                                                    {field.toLowerCase().includes('video') ? <Video className="w-4 h-4 text-primary" /> : 
                                                     field.toLowerCase().includes('image') || field.toLowerCase().includes('photo') ? <ImageIcon className="w-4 h-4 text-primary" /> :
                                                     <Sparkles className="w-4 h-4 text-primary" />}
                                                </div>
                                                <div>
                                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50 block"> {field.replace(/([A-Z])/g, ' $1').trim()}</label>
                                                    <span className="text-[8px] font-bold text-white/20 uppercase tracking-widest">Digital Asset</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        {field.toLowerCase().includes('video') ? (
                                            <div className="space-y-4">
                                                <div className="relative group/input">
                                                    <input
                                                        ...
                                                        placeholder="Vimeo / Drive / S3 URL..."
                                                        className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 text-xs text-white/80 placeholder:text-white/10 focus:border-primary/40 focus:bg-primary/5 transition-all outline-none"
                                                    />
                                                    <button
                                                        onClick={() => document.getElementById(`file-${field}`)?.click()}
                                                        className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-white/5 hover:bg-white/10 rounded-lg text-white/40 hover:text-primary transition-all"
                                                    >
                                                ...
                                                <div className="relative aspect-video rounded-xl overflow-hidden bg-black/60 border border-white/5 group/preview">
                                                    {giftData[activePage.id]?.[field] ? (
                                                        ...
                                                        <div className="absolute inset-0 flex flex-col items-center justify-center text-white/10 gap-3 border-2 border-dashed border-white/5 rounded-xl">
                                                            <div className="w-12 h-12 rounded-full bg-white/[0.02] flex items-center justify-center">
                                                                <Video className="w-6 h-6" />
                                                            </div>
                                                            <span className="text-[9px] font-black uppercase tracking-[0.3em]">Awaiting Visual Signal</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                                ...
                                                <div className="relative aspect-[4/3] rounded-xl overflow-hidden bg-black/60 border border-white/5">
                                                    {giftData[activePage.id]?.[field] ? (
                                                        ...
                                                        <div className="absolute inset-0 flex flex-col items-center justify-center text-white/10 gap-3 border-2 border-dashed border-white/5 rounded-xl">
                                                            ...
                                                            <span className="text-[9px] font-black uppercase tracking-[0.3em]">No Image Linked</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <textarea
                                                ...
                                                onChange={(e) => updateField(activePage.id, field, e.target.value)}
                                                rows={5}
                                                className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-4 text-xs text-white focus:border-primary/40 focus:bg-primary/5 transition-all outline-none resize-none font-medium leading-relaxed placeholder:text-white/10"
                                                placeholder={`Enter the message for ${field.toLowerCase()}...`}
                                            />
                                            ...
                                            <input
                                                ...
                                                value={giftData[activePage.id]?.[field] || ""}
                                                onChange={(e) => updateField(activePage.id, field, e.target.value)}
                                                className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-4 text-xs text-white focus:border-primary/40 focus:bg-primary/5 transition-all outline-none font-mono"
                                            />
                                        ) : (
                                            <input
                                                type="text"
                                                value={giftData[activePage.id]?.[field] || ""}
                                                onChange={(e) => updateField(activePage.id, field, e.target.value)}
                                                className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-4 text-xs text-white focus:border-primary/40 focus:bg-primary/5 transition-all outline-none"
