...
import {
  ...
  CreditCard, Users, Code, AlertCircle, Instagram, ScanLine
} from "lucide-react";
...
const Dashboard = () => {
  ...
  const [latestAlbumId, setLatestAlbumId] = useState<string | null>(null);
  ...
  const loadLatestAlbum = async () => {
    // Fetch user's latest AR album to auto-link the "Scan Now" button
    ...
    if (session) {
      const { data } = await supabase
        ...
        .limit(1)
        .single();
      if (data) setLatestAlbumId(data.id);
    }
  };
  ...
  const verifyPaymentStatus = async (purchase: UserPurchase) => {
    const orderId = purchase.cf_order_id || purchase.transaction_id;
    if (!orderId) {
      toast({ title: "Verification Failed", description: "No order ID found for this purchase.", variant: "destructive" });
      return;
    }

    toast({ title: "Checking Payment Status...", description: "Talking to Cashfree servers..." });

    try {
      const response = await fetch(`/api/verify-cashfree-order?orderId=${orderId}`);
      ...
      if (data.order_status === 'PAID') {
        ...
        toast({ title: "Payment Verified! 🎉", description: "Your template is now unlocked." });
        setPurchases(prev => prev.map(p => p.id === purchase.id ? { ...p, status: 'approved' } : p));
      } else {
        toast({ 
          title: "Payment Not Completed", 
          description: `Status: ${data.order_status || 'NOT PAID'}. Please complete the payment.`,
          variant: data.order_status === 'FAILED' ? 'destructive' : 'default'
        });
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-x-hidden">
      ...
      <div
        ...
        className="fixed top-[-10%] left-[-10%] w-[50%] h-[50%] bg-primary/15 blur-[150px] rounded-full pointer-events-none animate-pulse"
      />
      <div
        style={{ transform: 'translateZ(0)', willChange: 'opacity', backfaceVisibility: 'hidden' }}
        className="fixed bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-[#f04299]/10 blur-[150px] rounded-full pointer-events-none animate-pulse delay-700"
      />
      <div
        style={{ transform: 'translateZ(0)', willChange: 'opacity', backfaceVisibility: 'hidden' }}
        className="fixed top-[20%] right-[-5%] w-[30%] h-[30%] bg-emerald-500/10 blur-[120px] rounded-full pointer-events-none"
      />
      ...
      <header className="sticky top-0 z-50 glass-header px-6 py-4 flex items-center justify-between border-b border-white/5">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate(latestAlbumId ? `/scan?albumId=${latestAlbumId}` : '/scan')}
            className="relative w-10 h-10 rounded-xl glass-card-static flex items-center justify-center text-muted-foreground hover:text-white hover:border-white/20 transition-all group"
            title="Scan AR Gift"
          >
            <ScanLine className="w-5 h-5 group-hover:text-primary transition-colors" />
          </button>
                                ...
                                <div className={`glass-card flex flex-col h-full ${isLocked ? 'opacity-75' : 'cursor-pointer hover:border-primary/50'} transition-all duration-500 overflow-hidden shadow-2xl`}>
                                  ...
                                  {isLocked && purchase?.status !== 'rejected' && (
                                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-10 flex flex-col items-center justify-center p-6 text-center">
                                      ...
                                      <button 
                                        onClick={() => verifyPaymentStatus(purchase!)}
                                        className="mb-4 px-4 py-2 bg-primary text-white text-xs font-black rounded-full hover:scale-105 transition-transform flex items-center gap-2"
                                      >
                                        <History className="w-3 h-3" />
                                        Verify Status
                                      </button>

                                      <div className="flex items-center gap-2 text-primary text-[10px] font-bold bg-primary/10 px-4 py-2 rounded-full">
                                        <Clock className="w-3 h-3" />
                                        Automatic within 2 hours
                                      </div>
                                    </div>
                                  )}
